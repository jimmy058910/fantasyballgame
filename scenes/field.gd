...
# Uses variable names matching node names Team0_EndZone / Team1_EndZone
extends Node2D
...
const TEAM_0_START_X: float = -300.0  # Right side
const TEAM_1_START_X: float = 300.0 # Left side
...
func reset_play():
	...
	for player in players:
		...
		var start_x_base = TEAM_1_START_X if player.team_id == 1 else TEAM_0_START_X
		...
		player.global_position = Vector2(start_x_base, y_pos)
		...
		if player.has_method("reset_state"):
			...
			printerr("Player %s missing reset_state method!" % player.name)

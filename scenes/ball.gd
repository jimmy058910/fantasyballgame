...
func _ready():
    ...
    if pickup_area != null:
        ...
        # else: print_debug("Ball: PickupArea signal already connected.") # Reduce noise
...
func _on_pickup_area_body_entered(body: Node):
    ...
    if not body or not pickup_area or not body.is_in_group("players"):
        if _is_arriving_from_pass: _is_arriving_from_pass = false; return
    
    # print("--- Pickup Area Entered by: ", body.name, " ---") # Reduce noise
    ...
    if _is_arriving_from_pass:
        # print_debug("Attempting pass/kick reception for %s" % body.name) # Reduce noise
        ...
        var norm_catch = float(catcher_catching_stat - 1) / float(MAX_CATCHING_STAT - 1) if MAX_CATCHING_STAT > 1 else float(1.0) # Ternary fix
        var catch_chance = lerp(BASE_CATCH_CHANCE, MAX_CATCH_CHANCE, norm_catch)
        var adjustment = 0.0
        if is_instance_valid(intended_receiver):
            if body == intended_receiver: adjustment = 0.10 # print_debug("  - Target Match! Applying catch bonus.")
            else: adjustment = -0.10 # print_debug("  - Interception attempt! Applying catch penalty.")
            catch_chance = clamp(catch_chance + adjustment, 0.0, 1.0)
        # print_debug("  - Catching: %d -> Norm: %.2f -> Final Chance: %.2f" % [catcher_catching_stat, norm_catch, catch_chance]) # Reduce noise
        if randf() < catch_chance: print_debug("Catch Successful by %s!" % body.name); pickup_allowed = true
        else: print_debug("Catch FAILED! Dropped by %s." % body.name); var bv = body.velocity * -0.3 + Vector2(randf_range(-30,30),randf_range(-30,-100)); call_deferred("set_loose",bv); pickup_allowed=false; return
        intended_receiver = null
    else:
        # print_debug("Attempting loose ball pickup for %s" % body.name) # Reduce noise
        pickup_allowed = true; intended_receiver = null

    if pickup_allowed and current_possessor == null and not freeze and pickup_area.monitoring and is_player and not is_knocked:
        # print_debug("BALL AREA DEBUG: Pickup conditions MET for %s" % body.name) # Reduce noise
        ...
        print("BALL SCRIPT: Picked up by %s." % body.name) # Key event
...
func set_loose(bounce_dir_velocity: Vector2 = Vector2.ZERO):
    ...
    current_possessor = null; intended_receiver = null
    set_deferred("freeze", false)
    pass_reception_timer = MIN_AIR_TIME_BUFFER * 0.75 # Bobble timer
    _is_arriving_from_pass = true
    # print_debug("BALL SCRIPT: set_loose() - Initiating brief 'bobble' (%.2f sec)" % pass_reception_timer) # Reduce noise
    if pickup_area != null:
        if pickup_area.monitoring: pickup_area.set_deferred("monitoring", false)
    var impulse_dir = Vector2.RIGHT
    if bounce_dir_velocity.length_squared() > 0.01: impulse_dir = bounce_dir_velocity.normalized() * -1.0
...
func initiate_pass(passer: Node, target_teammate: Node): # Takes the target NODE now
    ...
    if pickup_area != null:
        ...
        # print_debug("BALL SCRIPT: initiate_pass() - Monitoring forced OFF") # Reduce noise
    ...
    # print("BALL SCRIPT: Passing towards (clamped): %s" % str(final_target_pos.round())) # Reduce noise
    ...
    if distance > 1.0: # Check threshold to avoid normalizing zero vector
        ...
        if effective_pass_speed > 0:
            pass_reception_timer = MIN_AIR_TIME_BUFFER + ((distance / effective_pass_speed) * 1.05 + 0.05)
            print("BALL SCRIPT: initiate_pass() - Reception timer: %.2f sec" % pass_reception_timer)
        else:
            pass_reception_timer = MIN_AIR_TIME_BUFFER + 0.1 # Ensure minimum even if speed is somehow zero
        ...
        if pickup_area != null: 
            ...
            print("BALL SCRIPT: Pass failed (too close) - PickupArea monitoring ON")
...
func initiate_kick(kicker: Node, target_destination: Vector2):
    ...
    if pickup_area != null:
        pickup_area.monitoring = false
        # print_debug("BALL SCRIPT: initiate_kick() - Monitoring forced OFF") # Reduce noise
    ...
    final_target_pos.x = clamp(final_target_pos.x, -field_half_width + field_margin, field_half_width - field_margin)
    final_target_pos.y = clamp(final_target_pos.y, -field_half_height + field_margin, field_half_height - field_margin)
    # print("BALL SCRIPT: Kicking towards (clamped): %s" % str(final_target_pos.round())) # Reduce noise
    ...
    if distance > 1.0:
        ...
        if effective_kick_speed > 0:
            pass_reception_timer = MIN_AIR_TIME_BUFFER + ((distance / effective_kick_speed) * 1.05 + 0.05)
            print("BALL SCRIPT: initiate_kick() - Reception timer: %.2f sec" % pass_reception_timer)
        else:
            pass_reception_timer = MIN_AIR_TIME_BUFFER + 0.1 # Ensure minimum
            printerr("Ball Error: Effective kick speed is zero!")
    else:
        # Target too close, just drop it
        print("BALL SCRIPT: Kick target too close after clamping, dropping ball.")
        linear_velocity = Vector2.ZERO
        if pickup_area != null: 
            pickup_area.monitoring = true
            print("BALL SCRIPT: Kick failed (too close) - PickupArea monitoring ON")
